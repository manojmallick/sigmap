...
description: SigMap evaluated against three RAG quality dimensions. Answer correctness 53% vs ~10%. Faithfulness 100% vs 8%. Hallucination risk 0% vs 75%. 90 tasks, 18 repos, fully reproducible.
head:
  - - meta
    - property: og:title
  ...
  - - meta
    - property: og:description
      content: "Answer correctness: 53% with SigMap vs ~10% without. Faithfulness: 100% grounded. Hallucination risk: 75% → 0%. 90 tasks across 18 real repos."
  - - meta
    - property: og:url
...
**✔ 5× better answers** — correct answers: 10% → **53%**  
**✔ 41% fewer prompts** — task prompt reduction measured on saved benchmark data  
...
|---|---|---|
| Right file found | 13.6% of the time | **80.0%** of the time |
| Prompts needed per task | 2.84 avg | **1.67 avg** |
| Answers from wrong context | 87% | **20%** |
| Code symbols hidden from AI | 75% | **0%** |
...
<div style="background:var(--vp-c-bg-soft);border-radius:10px;padding:1.1rem 1.2rem">
  ...
  <div style="display:flex;justify-content:space-between;align-items:center;margin:6px 0">
    ...
    <div style="flex:1;margin:0 8px;background:var(--vp-c-bg-mute);border-radius:3px;height:14px"><div style="background:#22c55e;height:14px;border-radius:3px;width:53%"></div></div>
    <span style="font-size:0.82em;color:#22c55e;min-width:32px;text-align:right">53%</span>
  </div>
...
<div style="background:var(--vp-c-bg-soft);border-radius:10px;padding:1.1rem 1.2rem">
  ...
  <div style="display:flex;justify-content:space-between;align-items:center;margin:6px 0">
    <span style="font-size:0.8em;color:var(--vp-c-text-2)">Without</span>
    <div style="flex:1;margin:0 8px;background:var(--vp-c-bg-mute);border-radius:3px;height:14px"><div style="background:#ef4444;height:14px;border-radius:3px;width:25%"></div></div>
    <span style="font-size:0.82em;color:#ef4444;min-width:32px;text-align:right">25%</span>
  </div>
  <div style="display:flex;justify-content:space-between;align-items:center;margin:6px 0">
    <span style="font-size:0.8em;color:var(--vp-c-text-2)">With</span>
...
<div style="background:var(--vp-c-bg-soft);border-radius:10px;padding:1.1rem 1.2rem">
  ...
  <div style="display:flex;justify-content:space-between;align-items:center;margin:6px 0">
    <span style="font-size:0.8em;color:var(--vp-c-text-2)">Without</span>
    <div style="flex:1;margin:0 8px;background:var(--vp-c-bg-mute);border-radius:3px;height:14px"><div style="background:#ef4444;height:14px;border-radius:3px;width:75%"></div></div>
    <span style="font-size:0.82em;color:#ef4444;min-width:32px;text-align:right">75%</span>
  </div>
  <div style="display:flex;justify-content:space-between;align-items:center;margin:6px 0">
    <span style="font-size:0.8em;color:var(--vp-c-text-2)">With</span>
...
<div style="flex:1;min-width:220px;background:var(--vp-c-bg-soft);border-radius:10px;padding:1.2rem 1.5rem;border:2px solid #7c6af7">
  ...
  <div style="font-size:2.4em;font-weight:700;color:#7c6af7">1.67</div>
  <div style="font-size:0.85em;color:var(--vp-c-text-2)">avg prompts to answer</div>
  <div style="margin-top:.8rem;font-size:0.85em;color:#7c6af7">80.0% hit@5 · 53% rank-1</div>
</div>
...
<div style="flex:1;min-width:220px;background:var(--vp-c-bg-soft);border-radius:10px;padding:1.2rem 1.5rem">
  ...
  <div style="font-size:2.4em;font-weight:700;color:#22c55e">−41%</div>
  <div style="font-size:0.85em;color:var(--vp-c-text-2)">fewer prompts needed</div>
  <div style="margin-top:.8rem;font-size:0.85em;color:#22c55e">5.9× lift in context relevance</div>
</div>
...
<div style="display:flex;align-items:center;gap:10px;margin:6px 0">
  ...
  <div style="position:relative;flex:1;background:var(--vp-c-bg-soft);border-radius:4px;height:22px">
    <div style="background:#7c6af7;height:22px;border-radius:4px;width:80%"></div>
  </div>
  <span style="font-size:0.85em;color:#7c6af7;width:48px">80.0%</span>
</div>
</div>

| | Without SigMap | With SigMap | Change |
|---|---|---|---|
| **Avg prompts to answer** | 2.84 | 1.67 | **−41%** |
| **Hit@5 (context relevance)** | 13.6% | 80.0% | **+5.9×** |
| **Context in rank 1** | ~1% | 53% | **+52 pts** |
...
context 80% of the time, usually at rank 1, resolving the task in a single prompt.
:::
...
<div style="margin:8px 0"><div style="display:flex;justify-content:space-between;margin-bottom:3px"><span>rails <span style="font-size:0.8em;color:var(--vp-c-text-3)">(1,179 files)</span></span><span style="color:var(--vp-c-text-3)">60% vs 0.4% random · 142×</span></div><div style="display:flex;gap:4px;align-items:center"><div style="background:var(--vp-c-bg-soft);flex:1;border-radius:3px;height:10px;overflow:hidden"><div style="background:#ef4444;height:10px;width:0.4%"></div></div></div><div style="display:flex;gap:4px;align-items:center;margin-top:2px"><div style="background:var(--vp-c-bg-soft);flex:1;border-radius:3px;height:10px;overflow:hidden"><div style="background:#7c6af7;height:10px;width:60%"></div></div></div></div>
...
<div style="margin:8px 0"><div style="display:flex;justify-content:space-between;margin-bottom:3px"><span>serilog</span><span style="color:var(--vp-c-text-3)">60% vs 5% random · 11.9×</span></div><div style="display:flex;gap:4px;align-items:center"><div style="background:var(--vp-c-bg-soft);flex:1;border-radius:3px;height:10px;overflow:hidden"><div style="background:#ef4444;height:10px;width:5%"></div></div></div><div style="display:flex;gap:4px;align-items:center;margin-top:2px"><div style="background:var(--vp-c-bg-soft);flex:1;border-radius:3px;height:10px;overflow:hidden"><div style="background:#7c6af7;height:10px;width:60%"></div></div></div></div>
...
<div style="margin:8px 0"><div style="display:flex;justify-content:space-between;margin-bottom:3px"><span>vapor</span><span style="color:var(--vp-c-text-3)">40% vs 3.8% random · 10.5×</span></div><div style="display:flex;gap:4px;align-items:center"><div style="background:var(--vp-c-bg-soft);flex:1;border-radius:3px;height:10px;overflow:hidden"><div style="background:#ef4444;height:10px;width:3.8%"></div></div></div><div style="display:flex;gap:4px;align-items:center;margin-top:2px"><div style="background:var(--vp-c-bg-soft);flex:1;border-radius:3px;height:10px;overflow:hidden"><div style="background:#7c6af7;height:10px;width:40%"></div></div></div></div>
...
**Correct: 53%** — AI received the exact file it needed, in first position. One prompt, done.\
**Partial: 27%** — Right file was present somewhere in context. AI can answer, but may need nudging.\
**Wrong: 20%** — Right file was never provided. AI answered from unrelated code.\
**Hallucination risk: 75%** — Fraction of codebase symbols invisible to AI without SigMap.
:::
...
<div style="margin:10px 0">
  <div style="display:flex;justify-content:space-between;font-size:0.85em;margin-bottom:4px">
    ...
    <span style="color:#22c55e">48 / 90 tasks</span>
  </div>
  <div style="background:var(--vp-c-bg-soft);border-radius:6px;height:28px;overflow:hidden">
    <div style="background:#22c55e;height:28px;width:53.3%;display:flex;align-items:center;padding-left:10px;font-size:0.82em;color:#fff;font-weight:600">53.3%</div>
  </div>
</div>
...
<div style="margin:10px 0">
  <div style="display:flex;justify-content:space-between;font-size:0.85em;margin-bottom:4px">
    ...
    <span style="color:#f59e0b">24 / 90 tasks</span>
  </div>
  <div style="background:var(--vp-c-bg-soft);border-radius:6px;height:28px;overflow:hidden">
    <div style="background:#f59e0b;height:28px;width:26.7%;display:flex;align-items:center;padding-left:10px;font-size:0.82em;color:#fff;font-weight:600">26.7%</div>
  </div>
</div>
...
<div style="margin:10px 0">
  <div style="display:flex;justify-content:space-between;font-size:0.85em;margin-bottom:4px">
    ...
    <span style="color:#ef4444">18 / 90 tasks</span>
  </div>
  <div style="background:var(--vp-c-bg-soft);border-radius:6px;height:28px;overflow:hidden">
    <div style="background:#ef4444;height:28px;width:20%;display:flex;align-items:center;padding-left:10px;font-size:0.82em;color:#fff;font-weight:600">20.0%</div>
  </div>
</div>
...
|---|---|---|---|
| **Correct** | Target file at rank 1 — full context, direct answer | 48 | 53.3% |
| **Partial** | Target file at rank 2–5 — context present but not leading | 24 | 26.7% |
| **Wrong** | Target file not in top 5 — AI answers from wrong context | 18 | 20.0% |

9 repos out of 18 scored **100% hit@5**. Large repos (rust-analyzer, abseil-cpp, riverpod, laravel, akka, vue-core) all hit 100%, confirming SigMap's gains scale with codebase size.

---

## 3. Hallucination risk proxy

Without SigMap, **75% of codebase symbols are hidden from the AI**. The AI can only see
...
<div style="flex:1;min-width:200px;background:var(--vp-c-bg-soft);border-radius:10px;padding:1.2rem 1.5rem">
  <div style="font-size:0.8em;text-transform:uppercase;letter-spacing:.08em;color:var(--vp-c-text-2);margin-bottom:.4rem">Without SigMap</div>
  <div style="font-size:2.4em;font-weight:700;color:#ef4444">75%</div>
  <div style="font-size:0.85em;color:var(--vp-c-text-2)">of symbols hidden from AI</div>
  <div style="margin-top:.6rem;font-size:0.82em;color:#ef4444">47,675 dark symbols</div>
</div>
...
<div style="flex:1;min-width:200px;background:var(--vp-c-bg-soft);border-radius:10px;padding:1.2rem 1.5rem;border:2px solid #7c6af7">
  ...
  <div style="margin-top:.6rem;font-size:0.82em;color:#7c6af7">16,114 grounded signatures</div>
</div>
...
| **Dark symbols (hidden)** | 47,675 | 0 |
| **Grounded symbols** | 16,114 | 16,114 |
| **Hallucination risk zone** | **75%** | **0%** |
...
|---|---|---|:---:|:---:|
| **Answer Correctness** | Does the AI receive the file that makes a correct answer possible? | Rank-1 retrieval hit across 90 tasks | ~10% | **53%** |
| **Faithfulness** | Are the AI's responses grounded in actual indexed code, not invented symbols? | % of codebase symbols indexed (grounded vs dark) | 25% grounded | **100%** grounded |
| **Hallucination Risk** | What fraction of the codebase is invisible to the AI — making hallucination probable? | Dark symbols / total symbols | **75%** | **0%** |
...
**Hallucination Risk** is what makes the other two metrics intuitive: 75% of codebase symbols
...
| **rails** | Ruby | 1,193 | 99.5% | 60% | 2 / 1 / 2 | +1071% | 1,092 | $1,119 | B |
| **axios** | TS | 99 | 94.3% | 60% | 2 / 1 / 2 | FITS | 0 | $75 | C |
| **rust-analyzer** | Rust | 675 | 99.8% | 100% | 4 / 1 / 0 | +2659% | 651 | $2,644 | **A** |
| **abseil-cpp** | C++ | 703 | 99.7% | 100% | 3 / 2 / 0 | +1734% | 665 | $1,756 | **A** |
| **serilog** | C# | 195 | 96.4% | 60% | 1 / 2 / 2 | +53% | 68 | $141 | C |
...
| **vapor** | Swift | 134 | 96.2% | 40% | 1 / 1 / 3 | +34% | 34 | $124 | C |
| **vue-core** | Vue | 249 | 98.0% | 100% | 1 / 4 / 0 | +234% | 175 | $314 | **A** |
| **svelte** | Svelte | 370 | 98.2% | 60% | 0 / 3 / 2 | +242% | 262 | $323 | C |
| **fastify** | JS | 28 | 95.3% | 60% | 3 / 0 / 2 | FITS | 0 | $39 | C |
| **fastapi** | Python | 32 | 97.1% | 80% | 3 / 1 / 1 | +39% | 10 | $130 | B |
| **Average / Total** | 18 repos | — | **97.6%** | **80.0%** | 48 / 24 / 18 | 13/18 overflow | **5,047** | **$9,390** | — |
...
**Weakest repos (≤60% hit@5):** rails, spring-petclinic, axios, serilog, vapor, svelte, fastify. On these the wrong file was ranked outside the top 5 in 2–3 of 5 tasks. For spring-petclinic this is expected — with only 67 files the random baseline is already 39%; SigMap still lifts to 60%. For vapor the signature extractor finds the right module but Swift fragment coverage limits precision.

**Strongest:** rust-analyzer (675 files, 127× lift), laravel (1,536 files, 307× lift), abseil-cpp (703 files, 140× lift). The larger and denser the codebase, the more SigMap's token-based compression wins over random file selection.
...
|---|---|---|
| Avg prompts to answer | 2.84 | **1.67** (−41%) |
| Context hit@5 | 13.6% | **80.0%** (+5.9×) |
| Correct context (rank 1) | ~1% | **53%** |
| Wrong context | ~87% | **20%** |
| Hallucination risk zone | **75%** | **0%** (fully indexed) |
