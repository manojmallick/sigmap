...
description: SigMap generalizes across 21 repos, 33 languages, and multiple domains with 81.1% hit@5 in the latest saved v8.32.0 retrieval run.
head:
  - - meta
    - property: og:title
...
these 105 tasks is yes — 81.1% hit@5 with no per-repo tuning in the latest saved v8.32.0 run.
:::
...
If you want one number to carry into launch messaging, use the shared `v8.32.0` snapshot rather than an older per-page variant:
