...
description: SigMap generalizes across 21 repos, 33 languages, and multiple domains with 87.8% hit@5 in the latest saved v8.8.1 retrieval run.
head:
  - - meta
    - property: og:title
...
::: info Official v8.8.1 benchmark snapshot
...
these 90 tasks is yes — 88% hit@5 with no per-repo tuning in the latest saved v8.8.1 run.
:::
...
If you want one number to carry into launch messaging, use the shared `v8.8.1` snapshot rather than an older per-page variant:
