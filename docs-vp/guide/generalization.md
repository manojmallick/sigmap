...
description: SigMap generalizes across 21 repos, 33 languages, and multiple domains with 87.8% hit@5 in the latest saved v8.13.0 retrieval run.
head:
  - - meta
    - property: og:title
...
::: info Official v8.13.0 benchmark snapshot
**Benchmark ID:** sigmap-v8.13-main &nbsp;·&nbsp; **Date:** 2026-07-11 (with R language)
...
these 90 tasks is yes — 88% hit@5 with no per-repo tuning in the latest saved v8.13.0 run.
:::
...
If you want one number to carry into launch messaging, use the shared `v8.13.0` snapshot rather than an older per-page variant:
